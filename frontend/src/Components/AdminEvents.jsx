...
const AdminEvents = () => {
  ...
  // Group users by club name for display
  const getParticipantsByClub = (clubName) => {
    return users.filter((user) => user.club.toLowerCase() === clubName.toLowerCase());
  };
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      {events.map((event) => {
        const participants = getParticipantsByClub(event.club);
        return (
          <div
            key={event._id}
